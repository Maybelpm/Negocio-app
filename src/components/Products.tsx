...
const Products: React.FC<ProductsProps> = ({ products, setProducts }) => {
  ...
  // Create product and optional image upload
    ...
    <div className="space-y-6">
      <div className="bg-gray-800/50 p-6 rounded-2xl shadow-lg">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <input
            type="text" placeholder="Nombre" value={newProduct.name}
            onChange={e => setNewProduct({ ...newProduct, name: e.target.value })}
            className="p-3 rounded-lg bg-gray-700 text-white"
          />
          <input
            type="text" placeholder="Categoría" value={newProduct.category}
            onChange={e => setNewProduct({ ...newProduct, category: e.target.value })}
            className="p-3 rounded-lg bg-gray-700 text-white"
          />
          <input
            type="number" placeholder="Precio" value={newProduct.price}
            onChange={e => setNewProduct({ ...newProduct, price: +e.target.value })}
            className="p-3 rounded-lg bg-gray-700 text-white"
          />
          <input
            type="number" placeholder="Stock" value={newProduct.stock}
            onChange={e => setNewProduct({ ...newProduct, stock: +e.target.value })}
            className="p-3 rounded-lg bg-gray-700 text-white"
          />
          <textarea
            placeholder="Descripción" value={newProduct.description}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {products.map(product => (
          <div key={product.id} className="bg-gray-800/50 p-4 rounded-2xl shadow flex flex-col justify-between">
            <div>
              <img src={product.imageurl || `https://picsum.photos/seed/${product.id}/200`} alt={product.name}
            ...
            <div className="mt-4 flex justify-between">
              ...
              >Eliminar</button>
              <button onClick={() => {/* modal de edición */}}
...
act, { useState, useEffect } from 'react';
import { Product } from '../types';
import { supabase } from '@/lib/supabaseClient';
